...
class TestPickupCoverage:
    ...
    def test_valid_spritesheet_sets_image(self):
        ...
        with patch("src.entities.pickup.SpriteSheet") as M:
            ...
            item = PickupItem(
                pos=(0, 0), groups=[], item_id="h", sprite_sheet="herb.png", quantity=1
            )
            assert item.image is not None

    def test_image_rescaled_when_width_differs_from_tile_size(self):
        """Ligne 51 : smoothscale déclenché quand la frame n'est pas TILE_SIZExTILE_SIZE."""
        ...
        with patch("src.entities.pickup.SpriteSheet") as M:
            ...
            item = PickupItem(
                pos=(0, 0), groups=[], item_id="coin", sprite_sheet="coin.png", quantity=1
            )
