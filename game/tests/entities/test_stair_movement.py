...
class TestStairMovementUnit:
    ...
    def test_ut_008_start_move_stair_right_input_diagonal_unmapped(self, setup_map_manager):
        ...
        mm.get_vertical_move_props = MagicMock(
            side_effect=lambda x, y: current_props if (x in (1, 2) and y in (0, 1)) else None
        )
        entity = BaseEntity(pos=(48, 48))
        entity.speed = 200
        mm.get_direction_flags = MagicMock(return_value=["any"])
    ...
    def test_ut_014_interpolation(self):
        ...
        entity.stair_move_distance = (
            pygame.math.Vector2(80, 48) - pygame.math.Vector2(48, 48)
        ).magnitude()
    ...
    def test_ut_016_vertical_move_map_config(self):
        """UT-016: MOVEMENT_BEHAVIORS has correct behavior configuration."""
        assert hasattr(Settings, "MOVEMENT_BEHAVIORS")
        assert Settings.MOVEMENT_BEHAVIORS["stair"].move_map[((1, 0), "right")] == (1, -1)
        assert Settings.MOVEMENT_BEHAVIORS["stair"].move_map[((-1, 0), "right")] == (-1, 1)
        assert Settings.MOVEMENT_BEHAVIORS["stair"].move_map[((1, 0), "left")] == (1, 1)
        assert Settings.MOVEMENT_BEHAVIORS["stair"].move_map[((-1, 0), "left")] == (-1, -1)

    def test_ut_021_down_right_stair_descending(self, setup_map_manager):
        """UT-021: down,right stair descending (Top is Top-Left, Bottom is Bottom-Right)."""
        mm = setup_map_manager()
        entity = BaseEntity(pos=(48, 48))
        entity.speed = 200
        mm.get_direction_flags = MagicMock(return_value=["any"])
        ...
        # Entering from top floor (left of stair) going right (1, 0)
        # Stair Top is at (1, 1), half=True
        # Stair Bottom is at (2, 2), half=False
        current_props = {
        ...
        # Movement from Top Stair to Bottom Stair (dx=1)
        entity.direction = pygame.math.Vector2(1, 0)
    ...
    def test_ut_022_down_right_stair_ascending(self, setup_map_manager):
        """UT-022: down,right stair ascending (Bottom is Bottom-Right, Top is Top-Left)."""
        mm = setup_map_manager()
        entity = BaseEntity(pos=(48, 48))
        entity.speed = 200
        mm.get_direction_flags = MagicMock(return_value=["any"])
        ...
        # Movement from Bottom Stair to Top Stair (dx=-1)
        entity.direction = pygame.math.Vector2(-1, 0)
        entity._vertical_move = current_props
        entity.start_move()
    ...
    def test_ut_023_down_left_stair_climbing_exit(self, setup_map_manager):
        """UT-023: down,left stair climbing exit (minimizes Y jump to flat)."""
        mm = setup_map_manager()
        entity = BaseEntity(pos=(48, 48))
        entity.speed = 200
        mm.get_direction_flags = MagicMock(return_value=["any"])
        ...
        # Player at top stair (1, 1), offset 8, half=True
        current_props = {
        ...
        # Step-off target (2, 0) has no stair props
        mm.get_vertical_move_props = MagicMock(
            side_effect=lambda x, y: current_props if (x == 1 and y == 1) else None
        )
        ...
        entity.direction = pygame.math.Vector2(1, 0)
        entity._vertical_move = current_props
        entity.current_stair_offset = 8.0
        entity.start_move()

        # Target dir should be flat (1, 0) because abs(-8) < abs(-32 - 8)
        assert entity.direction == pygame.math.Vector2(1, 0)
        assert entity.is_moving is True

    def test_ut_024_down_left_stair_descending_exit(self, setup_map_manager):
        """UT-024: down,left stair descending exit (minimizes Y jump to diagonal)."""
        mm = setup_map_manager()
        entity = BaseEntity(pos=(48, 48))
        entity.speed = 200
        mm.get_direction_flags = MagicMock(return_value=["any"])
        ...
        # Player at bottom stair (1, 1), offset 24, half=False
        current_props = {
            "stair_direction": "down,left",
            "movement_type": "stair",
            "half": False,
        ...
        # Step-off target (0, 2) has no stair props
        mm.get_vertical_move_props = MagicMock(
            side_effect=lambda x, y: current_props if (x == 1 and y == 1) else None
        )
        ...
        entity.direction = pygame.math.Vector2(-1, 0)
        entity._vertical_move = current_props
        entity.current_stair_offset = 24.0
        entity.start_move()
