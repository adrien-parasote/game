...
breaking chests (128x172 sheet → 32px instead of 43px) and leviers (32x128 → 32px
...
class TestSpriteFrameHeightCalculation:
    ...
    def test_torch_frame_height_computed_from_sheet(self):
        """
        A torch spritesheet 32x256 with end_row=3 must call load_grid_by_size
        ...
        """
        # Sheet is 32px wide x 256px tall → 4 rows of 64px each
    ...
    def test_frame_count_matches_real_sheet_layout(self):
        """
        A 128x128 sheet with 32x32 frames must produce 16 frames (4 cols x 4 rows),
        ...
        assert len(entity.frames) == 16, f"Expected 16 frames (4x4), got {len(entity.frames)}"

    @pytest.mark.tc("SPRITE-U-03")
    def test_chest_sheet_correct_frame_height(self):
        """
        A chest spritesheet 128x172 with end_row=3 must produce frames of
    ...
    def test_get_frame_returns_sheet_height_not_tiled_height(self):
        """
        After loading a valid sheet (128x172, end_row=3), _get_frame(0) must return
