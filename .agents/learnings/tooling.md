...
> Domaine: `TOOL`
> Périmètre: Développement d'outils, scripts de conversion, Asset Convertor, générateurs.

## L-TOOL-001 · 2026-05-31 · Universal · UI Framework Python
**Contexte :** Outil Asset Convertor avec UI interactive.
...
**Pattern :** Toujours utiliser le pipeline Blob pour la génération de terrains. L'implémentation de référence est `tools/src/asset_convertor/core/converter_xp.py` + `converter_mv.py` + `tsx_generator.py` (l'ancien `scripts/autotiles/rpgmaker_blob_autotile_to_tiled.py` a été supprimé le 2026-06-04 et remplacé par ce pipeline).
...
**Pattern :** Extraire toutes les valeurs magiques dans un `constants.py` au sein du module d'outil (ex: `tools/src/asset_convertor/core/constants.py`) et l'importer dans l'UI et le core.

## L-TOOL-004 · 2026-06-03 · Universal · macOS Native Dock Icon for Python GUI
**Contexte :** Remplacement de l'icône par défaut (la "fusée" de Python) sur macOS pour l'application `asset_convertor` (Dear PyGui).
