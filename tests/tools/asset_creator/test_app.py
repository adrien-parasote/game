...
from tools.asset_creator.gui import app
...
def test_run_gui() -> None:
    ...
    with patch("tools.asset_creator.gui.app.regenerate_tileset", return_value=[]):
        app.run_gui()
...
def test_frame_tick() -> None:
    ...
    app._state = app.state_from_preset("grass", app._presets)
    app._canvas = app.CanvasState(cols=app.CANVAS_COLS, rows=app.CANVAS_ROWS)
    with patch("tools.asset_creator.gui.app._do_regenerate"):
        app._frame_tick()
    assert not app._pending_regen

def test_on_mode_change() -> None:
    with patch("tools.asset_creator.gui.app.dpg.get_value", return_value="standalone"), \
         patch("tools.asset_creator.gui.app._do_regenerate") as mock_regen:
        app._on_mode_change()
...
def test_on_clear_canvas() -> None:
    ...
    with patch("tools.asset_creator.gui.app._update_all_canvas") as mock_update:
        app._on_clear_canvas()
        assert not app._canvas.grid[0][0]
        mock_update.assert_called()

def test_on_param_change() -> None:
    with patch("tools.asset_creator.gui.app._do_regenerate") as mock_regen:
...
def test_on_preset_change() -> None:
    with patch("tools.asset_creator.gui.app._do_regenerate") as mock_regen:
        with patch("tools.asset_creator.gui.app.dpg.get_value", return_value="grass"):
                app._on_preset_change()
        assert app._pending_regen

def test_do_regenerate_autotile() -> None:
    app._state = app.state_from_preset("grass", app._presets)
    with patch("tools.asset_creator.gui.app.dpg.get_value", return_value="autotile"), \
         patch("tools.asset_creator.gui.app.regenerate_tileset", return_value=[]) as mock_regen, \
         patch("tools.asset_creator.gui.app._update_preview_texture"), \
         patch("tools.asset_creator.gui.app._update_all_canvas"), \
         patch("tools.asset_creator.gui.app._push_history"):
        app._do_regenerate()
        mock_regen.assert_called()
...
def test_do_regenerate_standalone() -> None:
    app._state = app.state_from_preset("grass", app._presets)
    with patch("tools.asset_creator.gui.app.dpg.get_value", return_value="standalone"), \
         patch("tools.asset_creator.gui.app.generate_standalone_tile", return_value=None) as mock_gen, \
         patch("tools.asset_creator.gui.app._update_preview_texture"), \
         patch("tools.asset_creator.gui.app._update_all_canvas"), \
         patch("tools.asset_creator.gui.app._push_history"):
        app._do_regenerate()
...
def test_on_export() -> None:
    ...
    with patch("tools.asset_creator.gui.app.dpg.get_value", return_value="autotile"), \
         patch("tools.asset_creator.gui.app.do_export_standalone", return_value="foo.png"), \
         patch("tools.asset_creator.gui.app.do_export_autotile", return_value=("foo.png", "foo.tsx")):
        app._on_export()
        # Should not crash

def test_sync_widgets_from_state() -> None:
    app._state = app.state_from_preset("grass", app._presets)
    with patch("tools.asset_creator.gui.app.dpg.set_value"):
...
def test_history_select() -> None:
    app._state = app.state_from_preset("grass", app._presets)
    with patch("tools.asset_creator.gui.app.dpg.set_value"), \
         patch("tools.asset_creator.gui.app.dpg.configure_item"):
        app._push_history()
        app._history_index = 0
        with patch("tools.asset_creator.gui.app._sync_widgets_from_state"), \
             patch("tools.asset_creator.gui.app._do_regenerate"):
