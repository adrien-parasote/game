...
# Mock pygame to allow pygame_preview.py to be imported
...
from tools.asset_creator.preview import pygame_preview
from PIL import Image

def test_pygame_preview_import() -> None:
    """Importing the pygame preview module succeeds with mocked pygame."""
    assert pygame_preview is not None

def test_run_preview_quit() -> None:
...
def test_run_preview_with_subtiles() -> None:
    ...
    with patch.object(mock_pygame.event, "get", return_value=[quit_event]):
        pygame_preview.run_preview(image, subtile_set=subtiles)
