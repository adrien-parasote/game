...
class PickupItem(BaseEntity):
    ...
    def __init__(self, pos: tuple, groups: list[pygame.sprite.Group], 
        ...
        if not os.path.exists(sheet_path):
            ...
            sheet_path = os.path.join(os.path.dirname(__file__), "..", "..", "assets", "images", "icons", sprite_sheet)
        ...
        if sheet.valid:
            ...
            if self.image.get_width() != Settings.TILE_SIZE:
                self.image = pygame.transform.smoothscale(self.image, (Settings.TILE_SIZE, Settings.TILE_SIZE))
        else:
            logging.error(f"PickupItem: Could not load sprite {sprite_sheet} from sprites or icons.")
