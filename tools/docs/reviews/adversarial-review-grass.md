...
| Check | Finding |
|-------|---------|
| `DEFAULT_PALETTES` schema | Grass spec says "5 colors per entry". Current `constants.py` has 4-color tuples. The grass spec claims this is a modification — OK. But `asset_creator_spec.md` does **not** mention `DEFAULT_PALETTES` at all (confirmed via search: zero hits). The cross-spec contract in the grass spec claims the producer is `asset_creator_spec.md`, but this is **phantom reference** — the contract is undocumented there. |
| `generate_texture` / `quantize_image` public interface | Grass spec declares these in its `## Public Interface` table pointing to `asset_creator_spec.md`. Confirmed via search: `asset_creator_spec.md` does not document these functions. Only `phase-1-simple-tiles.md` does, with the conflicting 4-value range. |
