...
> Strategic Blueprint: [`tools/docs/strategic/diagonal_wall_blueprint.md`](../strategic/diagonal_wall_blueprint.md#blueprint-2d-diagonal-wall-tile-transformation-utility)  
> Date: 2026-05-28  
...
The engine reads a flat wall source image of width $W$ and height $H$. It segments it vertically and performs column-by-column shearing:
- **Vertical Shear Translation Formula:**
  For direction `nw-se` (North-West to South-East):
  For each column $x \in [0, W-1]$ of the source image, all pixels in that column are shifted vertically downwards by $x$ pixels. 
  $$\text{Target}(x, y + x) = \text{Source}(x, y) \quad \forall y \in [0, H-1]$$
  
  For direction `ne-sw` (North-East to South-West):
  For each column $x \in [0, W-1]$ of the source image, all pixels in that column are shifted vertically downwards by $(W - 1) - x$ pixels.
  $$\text{Target}(x, y + (W - 1) - x) = \text{Source}(x, y) \quad \forall y \in [0, H-1]$$

- **Layer Composition Details:**
  To preserve pixel-art integrity, the transformation is applied column-by-column as integer-pixel translations using PIL's `paste()` interface. No float interpolation or sub-pixel coordinate scaling is allowed.
