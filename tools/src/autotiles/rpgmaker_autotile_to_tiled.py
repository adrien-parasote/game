...
_Q = {
    ...
    # RIGHT EDGE (straight left, connects T/B) -> col 5, rows 4-5
...
def _quarter_source(
    ...
    # Determine vertical and horizontal neighbors for the given corner
    if corner == "tl":
        v_neigh, h_neigh = top, left
        v_only_key, h_only_key = "lft", "top"
    elif corner == "tr":
        v_neigh, h_neigh = top, right
        v_only_key, h_only_key = "rgt", "top"
    elif corner == "bl":
        v_neigh, h_neigh = bottom, left
        v_only_key, h_only_key = "lft", "bot"
    elif corner == "br":
        v_neigh, h_neigh = bottom, right
        v_only_key, h_only_key = "rgt", "bot"
    else:
        raise ValueError(f"Unknown corner: {corner!r}")

    # Determine the key prefix
    if v_neigh and h_neigh:
        prefix = "inn"
    elif v_neigh:
        prefix = v_only_key
    elif h_neigh:
        ...
        prefix = "out" if mask_zero else "cvx"

    return _Q[f"{prefix}_{corner}"]
